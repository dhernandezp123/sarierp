...
export default function VentasPage() {
  ...
  const fetchActivities = useCallback(async () => {
    ...
    setLoading(false)
  }, [filterTipo, filterEtapa, filterPeriod, today])
  ...
  const handleSave = async () => {
    ...
    if (form.tipo_cliente === 'Nuevo' && !form.empresa_prospecto && !form.nombre_prospecto) {
      toast.error('Ingresa el nombre o empresa del prospecto')
      return
    }
    ...
    <div className="space-y-6">
                  ...
                  {ETAPAS.map((e) => (
                    <button
                      ...
                      onClick={() =>
                        setForm((f) => ({
                          ...f,
                          etapa_captacion: f.etapa_captacion === e.value ? '' : e.value,
                        }))
                      }
              ...
              <div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      className={fieldClass}
                      placeholder="Ej: Enviar cotización, Agendar visita..."
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          ...
          <div
            key={label}
            className={`flex items-center gap-4 rounded-2xl border p-4 ${bg}`}
          >
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-3 flex-1 min-w-0">
                          {/* Tipo icon */}
                          <span
                            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${TIPO_COLOR[a.tipo_actividad]}`}
                          >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {etapa && (
                                <span
                                  className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${etapa.color} ${etapa.bg}`}
                                >
                                  {etapa.value}
                                </span>
                              )}
                            ...
                            <div className="mt-0.5 flex flex-wrap items-center gap-3 text-xs text-slate-400 dark:text-slate-500">
                              ...
                              {a.profiles && (
                                <span>
                                  {a.profiles.nombre} {a.profiles.apellido}
                                </span>
                              )}
                            </div>
